...
void seleccion()
{
	...
	while(opci == 0)
	{
		...
		{
			case 1:
				system("transaccion.exe"); 
				break;
			case 2:
				system("mantenimiento con arreglo.exe"); 
				break;
			case 3:
				system("mantenimiento con lista.exe"); 
				break;
			case 4:
				system("consultas.exe"); 
				break;
			case 0:
...
int validacion(int mini, int maxi) //VALIDACION DE UN RANGO DE NUMEROS
{
	int auxi = -1;
	char cara = 0;
	while(auxi < mini || auxi > maxi) //CICLO QUE SE REPITE HASTA QUE SE INGRESE POR TECLADO UN NUMERO QUE ESTE DENTRO DEL RANGO DE VALORES DESDE EL PRIMER PARAMETRO HASTA EL SEGUNDO PARAMETRO DE LA FUNCION DE USUARIO
	{
		cara = getch(); //FUNCION QUE TOMA UN CARACTER INGRESADO POR EL USUARIO PERO NO LO MUESTRA POR PANTALLA
		auxi = cara - '0'; //OPERACION QUE CONVIERTE UN CARACTER NUMERICO EN UN ENTERO
	}
	printf("%d", auxi);
	return(auxi);
}
