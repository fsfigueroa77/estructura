...
struct sproveedor
{
	...
	char nruc[14]; //NUMERO DE RUC 
	char dire[40]; //DIRECCION
	char ciud[20]; //CIUDAD
	char tele[11]; //TELEFONO 
	char mail[25]; //EMAIL
	char esta; //ESTADO
};
...
void nuevo()//FUNCION DE INGRESO DE REGISTRO
{
	...
	etiqueta("Numero RUC........:",'I',15,16,0); 
	etiqueta("Direccion.........:",'I',15,18,0);
	etiqueta("Ciudad............:",'I',15,20,0);
	etiqueta("Telefono..........:",'I',15,22,0); 
	...
	strcpy(prov[inde].nruc, entrada_cadena_modificada(35, 16, 2, 13)); 
	strcpy(prov[inde].dire, entrada_cadena_modificada(35, 18, 3, 40));
	strcpy(prov[inde].ciud, entrada_cadena_modificada(35, 20, 1, 20)); 
	strcpy(prov[inde].tele, entrada_cadena_modificada(35, 22, 2, 10));
		...
		if(dato==prov[cont].codi)
		{
			...
			if(opci=='S')
			{
				...
				if(opci=='S')	
				{
					...
					strcpy(prov[cont].nruc,nruc); 
					strcpy(prov[cont].dire,dire); 
					strcpy(prov[cont].ciud,ciud);
					strcpy(prov[cont].tele,tele); 
...
void eliminar()
{
	...
	char nomb[20]; //NOMBRE DE PROVEEDOR
	char nruc[14]; //NUMERO DE RUC //DESABILITADA POR FUNCIONAMIENTO INESPERADO
	char dire[40]; //DIRECCION
	char ciud[20]; //CIUDAD
	char tele[11]; //TELEFONO
	char mail[25]; //EMAIL	
		...
		if(dato==prov[cont].codi)
		{
			...
			if(opci=='S')
			{
				while(cont<inde)
				{
					...
					strcpy(prov[cont].nruc,prov[cont+1].nruc);
					strcpy(prov[cont].dire,prov[cont+1].dire);
					strcpy(prov[cont].ciud,prov[cont+1].ciud);
					strcpy(prov[cont].tele,prov[cont+1].tele); 
...
void guardar() 
{
    ...
    if (fichero != NULL)
	{
        while (cont < inde)
		{
            ...
            strcpy(tele, prov[cont].tele); 

            strcpy(mail, prov[cont].mail);

            //fprintf(fichero, "%d\t%s\t%s\t%s\t%s\t%s\t%c\n", prov[cont].codi, nomb, nruc, dire, ciud, mail, prov[cont].esta);
