...
TEMA: MANTENIMIENTO CON LISTA
*/
//ARCHIVOS CABECERA
#include<stdio.h> 
#include<conio.h> 
#include<rosian.h> 
#pragma GCC diagnostic ignored "-Wwrite-strings" //LINEA DE CODIGO QUE OCULTA LOS ERRORES LEVES
//DECLARACION DE ESTRUCTURA CON NODO
struct spais
{
	int codi;
	char desc[25];
...
int main()
{
	LIST lpai = NULL; //INICIALIZACION DE UNA VARIABLE TIPO LISTA
	char opci;
	char cade[50];
	system("title ROSIAN PC [Pais]");
	system("mode con: cols=160 lines=40");
	system("title ROSIAN PC [Usuario]");
	cargar(lpai); //FUNCION CARGAR CON PASO DE PARAMETRO DEL TIPO LISTA
	pantalla("ASESORAMIENTO ACADEMICO <ROSIAN PC>","Ruc:1234567890001  Tel:0982788880","Guayaquil - Ecuador", "USUARIO");
	do
	{
		bp(4);
		consulta_general(lpai); //FUNCION CONSULTA GENERAL CON PASO DE PARAMETRO DEL TIPO LISTA
		opci=menu();
		switch(opci)
		{
			case '1':
...
void nuevo(LIST &list) //FUNCION NUEVO CON PASO DE PARAMETRO POR REFERENCIA DEL TIPO LISTA
{
	char opci;
	char cade[50];
	char auxi[50];
	...
	if(opci=='S')
	{
		...
		LIST lis1; //DECLARACION DE UNA VARIABLE LOCAL DEL TIPO LISTA
...
{
	LIST lis1; //DECLARACION DE VARIABLE LOCAL TIPO LISTA
	lis1=list; //IGUALACION DE LISTAS
	int band=0;
	int dato=0;
	int cont=0;
	int codi;
	...
	for(cont=0;cont<indi;cont++)
	{
		if(dato==lis1->codi)
		{
			...
			opci=toupper(getch()); //TRANSFORMA EN MAYUSCULA EL CARACTER INGRESADO POR TECLADO
			bp(3);
			if(opci=='S')
			{
				poscf(15,16);printf("                                              ");
...
void eliminar(LIST &list)
{
	...
	int vcol;
	char cade[50];
	LIST lis1; //DECLARACION DE UNA PRIMERA VARIABLE LOCAL TIPO LISTA
	LIST lis2; //DECLARACION DE UNA SEGUNDA VARIABLE LOCAL TIPO LISTA
	bp(2);
...
void consulta_individual(LIST list)//FUNCION QUE MUESTRA LOS DATOS DE UN REGISTRO
{
	...
	if(list==NULL) //CONDICION DE LA ESTRUCTURA DE CONTROL EN LA CUAL ENTRA A ESTA SI LA LISTA ES NULL
	{
		mensaje('X',"Lista vacia.");
	}
	else
...
/*
PREGUNTAS:
1.- ¿QUE HACE LA FUNCION DE PROGRAMA new()
2.- ¿PARA QUE SIRVE "->"?
3.- ¿POR QUE EN OCASIONES LAS VARIABLES TIPO LISTA SE IGUALAN A NULL?
4.- ¿POR QUE EN OCASIONES EL "->" VA ANTES DEL IGUAL Y EN OTRAS VA DESPUES DEL IGUAL?
5.- ¿POR QUE EN OCASIONES LAS LISTAS SE DECLARAN SOLAMENTE Y EN OTRAS SE DECLARAN Y SE INICIALIZAN EN NULL?
